...
## Introduction
This privacy policy covers the use of the 'Serva LinkCheck' (https://github.com/TrianguloY/Serva LinkCheck) Android application.

It may not be applicable to other software produced or released by TrianguloY (https://github.com/TrianguloY).

Serva LinkCheck when running does not collect any statistics, personal information, or analytics from its users, other than Android operating system built in mechanisms that are present for all the mobile applications.

Serva LinkCheck does not contain any advertising sdk, nor tracker of the user or his device.
...
Serva LinkCheck has the ability, at the user's request, to retrieve the URLs passing through the app by relying on the logging module. This is done without storing any identifiable user information, the data is only stored locally on the user's device;
...
* allows online url unshortening, upon user activation, relying on Unshorten cloud service. Unshorten user credentials (API key) is not required. Optionally, this service may store user information and data allowing identification; Please refer to Unshorten's privacy policy (https://unshorten.it/privacy-policy) for details on how they handle user data.
 ...
 <!-- Serva LinkCheck specific licenses of libraries used in the application can be accessed from About section. - Not useful actually -->

## Android permissions requested by the application
Note that Serva LinkCheck application requires the following android platform permissions:
